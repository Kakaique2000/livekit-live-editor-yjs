...
export default function ExcalidrawComponent() {
    ...
    React.useEffect(() => {
        console.debug("Setting up excalidraw component", api, excalidrawRef.current, localParticipant);
        if (!api || !excalidrawRef.current || !localParticipant) return;


        const color = pickFromArrayBySeed(localParticipant.localParticipant.identity, usercolors);
        console.debug("Setting local state field for user", localParticipant.localParticipant.identity, color);
        // ExcalidrawAwareness.setLocalStateField('user', {
        //     name: localParticipant.localParticipant.identity,
        //     color: color.color,
        //     light: color.light,
        // });
    ...
    return <div className='h-full w-full' ref={excalidrawRef}>
        <Excalidraw
            initialData={initData}
            excalidrawAPI={setApi}
            onPointerUpdate={binding?.onPointerUpdate} theme='dark' />
    </div>
}